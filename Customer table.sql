...
insert into product(Pname,Price,Company)
values('Iphone',2000,'Apple'),
('S22',1000,'Samsung'),
('Oneplus',9,3000,'Poco');
...
insert into orders (Ordername,Pid)
values('Firstorder',1);
...
set sql_safe_update =0;
 ...
 describe customers;
select count(*) as numberofLecturer,Cust_Country
from customers
group by Cust_Country
order by count(*);

select avg(Cust_Salary) as avgsalary ,Cust_Country
from customers
group by Cust_Country
order by avg(Cust_Salary);

select Cust_FirstName
from customers;

create index indx_customers
on customers(cust_FirstName);

show index from customers;

alter table customers
drop index indx_customers;
